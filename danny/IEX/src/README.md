...
This directory contains various C++ and Python files needed to download and parse IEX (Investors Exchange) data.
...
The resulting CSV file includes parsed trade reports and price level updates (see [Parser README](/group_03_project/server/parsers/IEX/README.md)). If these files become too large to manage, you can utilize iex_parser_split.cpp to divide the files based on the initial letter of the symbol.

**Download and Parse**
======================

The `download_and_parse.py` script automates the downloading of IEX DEEPs (Depth of Book) data for a specified range of dates and symbols, parses the downloaded pcap files, and cleans the data for further analysis. The download portion relies on the `download_dates` function, which is part of the `download_iex_pcaps.py` file (see src folder).
