...
#include "venue_arb_super_aggressive.h"
...
venue_arb_super_aggressive::venue_arb_super_aggressive(StrategyID strategyID,
...
venue_arb_super_aggressive::~venue_arb_super_aggressive() {}

void venue_arb_super_aggressive::DefineStrategyParams()
{
...
void venue_arb_super_aggressive::DefineStrategyCommands()
{
    commands().AddCommand(StrategyCommand(1, "Cancel All Orders"));
}

void venue_arb_super_aggressive::RegisterForStrategyEvents(StrategyEventRegister* eventRegister,
                                         DateType currDate)
{

}

void venue_arb_super_aggressive::OnResetStrategyState()
{
...
void venue_arb_super_aggressive::OnQuote(const QuoteEventMsg& msg)
{
...
void venue_arb_super_aggressive::EvaluateArb(const Instrument* inst)
{
...
void venue_arb_super_aggressive::AdjustPortfolio(const Instrument* inst, int trade_direction, MarketCenterID venue)
{
    ...
    int working_orders = orders().num_working_orders(inst);
    if (working_orders >= 10) {  // Allow up to 10 working orders per instrument
        if (debug_) {
            std::cout << "[SKIP] " << inst->symbol() 
                      << " has " << working_orders << " working orders (max 10)" << std::endl;
        }
        return;
    }
...
void venue_arb_super_aggressive::SendOrder(const Instrument* inst,
                         int trade_size,
...
void venue_arb_super_aggressive::OnStrategyCommand(const StrategyCommandEventMsg& msg)
{
    if (msg.command_id() == 1) {
...
void venue_arb_super_aggressive::OnParamChanged(StrategyParam& param)
{
