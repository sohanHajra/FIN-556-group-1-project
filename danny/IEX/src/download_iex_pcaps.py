...
class IexAvailableFiles(object):
    ...
    def fetch_available_files(target_data_feed, start_date=None, end_date=None):
        if target_data_feed != "TOPS" and target_data_feed != "DEEP" and target_data_feed != "DPLS":
            raise Exception("Invalid target_data_feed_type (%s); must be either TOPS, DEEP, or DPLS" % (target_data_feed))
