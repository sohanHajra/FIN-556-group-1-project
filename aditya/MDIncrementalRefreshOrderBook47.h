...
public:
    ...
    #pragma GCC diagnostic pop
        ...
        SBE_NODISCARD inline std::uint64_t debugBuf() const SBE_NOEXCEPT
        {
            return m_bufferLength;
        }
        
        SBE_NODISCARD inline std::uint64_t debugOff() const SBE_NOEXCEPT
        {
            return *m_positionPtr;
        }
        ...
        inline NoMDEntries &next()
{
    ...
    // DEBUG PRINT - SAFE, no new methods
    // std::cout << "[DEBUG] index=" << m_index 
    //           << " count=" << m_count 
    //           << " pos=" << *m_positionPtr 
    //           << " + blockLen=" << m_blockLength 
    //           << " = " << (*m_positionPtr + m_blockLength)
    //           << " vs bufLen=" << m_bufferLength << std::endl;
    
    m_offset = *m_positionPtr;
    if (SBE_BOUNDS_CHECK_EXPECT(((m_offset + m_blockLength) > m_bufferLength), false))
    {
        std::cout << "[CRASH] " << m_offset << " + " << m_blockLength 
                  << " > " << m_bufferLength << std::endl;
