...
if __name__ == "__main__":
    ...
    while current_date <= end_date:
        ...
        for file_path in matching_files:
            ...
            if split=="True":
                IEX_PARSER = os.getenv('IEX_PARSER_SPLIT', '/vagrant/parsers/IEX/src/iex_parser_split.out')
            else:
                IEX_PARSER = os.getenv('IEX_PARSER', '/vagrant/parsers/IEX/src/iex_parser.out')
            ONE_TICK_PARSED= os.getenv('ONE_TICK_PARSED', '/vagrant/parsers/IEX/data/one_tick_parsed')
            command2 =f"gunzip -d -c {file_path} | tcpdump -r - -w - -s 0 | {IEX_PARSER} /dev/stdin {ONE_TICK_PARSED}/{current_date_str_2} {symbol}"
